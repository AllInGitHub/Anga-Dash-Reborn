...
class FlxState extends FlxGroup
{
	...
	@:allow(backend.StfUtils.trans)
	var _trans:backend.StfUtils.TransitionData = null;
	...
	public function create():Void
	{
		...
		{
			case FADE:
				FlxG.cameras.fade(FlxColor.BLACK, _trans.duration / 2, true);
			default:
		}
	}
	...
	function tryUpdate(elapsed:Float):Void
	{
		...
		{
			#if FLX_DEBUG
			if (FlxG.keys.anyJustReleased(FlxG.debugger.toggleKeys))
